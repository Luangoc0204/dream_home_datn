...
class ChattingUserPage extends StatefulWidget {
  const ChattingUserPage({super.key, required this.currentUser});
  final User currentUser;

  @override
  State<ChattingUserPage> createState() => _ChattingUserPageState();
}

class _ChattingUserPageState extends State<ChattingUserPage> {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: StreamBuilder(
                stream: _chatViewModel.getChattingList(widget.currentUser.id),
  ...
  Widget _buildItemChat(DocumentSnapshot document) {
    ...
    bool isSender = (data['senderUserId'] == widget.currentUser.id);
    return GestureDetector(
      onTap: () {
        pushNewScreen(
          context,
          screen: DetailChatUserPage(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ClipOval(
                child: isSender
                    ? (data['receiverUserAvatar'] == null ? Image.asset(
                  ...
                  height: 50,
                  fit: BoxFit.cover,
                )) : (data['senderUserAvatar'] == null ? Image.asset(
                  'assets/img_avatar_empty.jpg',
                  width: 60,
                  height: 60,
                  fit: BoxFit.cover,
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  StreamBuilder(
                    stream: _chatViewModel.getLastMessage(data['senderUserId'], data['receiverUserId']),
                    builder: (context, snapshot) {
                      if (snapshot.hasError) {
                        return Text('Error ${snapshot.error}');
                      }
